...
from meme_api.configs import RPC, KEYPAIR
...
class SolanaTracker:
    ...
    def confirm_transaction(
        self, txid: str, max_retries: int = 60, retry_interval: float = 1.0
    ) -> str:
        retries = 0
        while retries < max_retries:
            try:
                resp = self.connection.get_signature_statuses(
                    [Signature.from_string(txid)], True
                )
                if resp.value is not None and len(resp.value) > 0:
                    if (
                        self.debug
                        and resp.value[0] is not None
                        and resp.value[0].confirmations
                    ):
                        print(f"Confirmations: {resp.value[0].confirmations}")
                    if (
                        resp.value[0] is not None
                        and resp.value[0].confirmation_status is not None
                        and str(resp.value[0].confirmation_status)
        ...
        if retries >= max_retries:
            raise TimeoutError(
                f"Transaction {txid} was not confirmed after {max_retries} retries"
            )
        else:
            return txid
